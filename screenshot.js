...
/*!
* html2canvas-pro 2.4.5 <https://yorickshan.github.io/html2canvas-pro/>
...
(function(global, factory) {
	...
	*/
	...
	const spreadShadowPath = (paths, spread) => {
		...
		const radii = corners.map((c) => ({
			x: shadowRadius(c.rx, spread),
			y: shadowRadius(c.ry, spread)
		}));
		const ratio = (size, sum) => sum > 0 ? size / sum : 1;
		const factor = Math.min(1, ratio(width, radii[0].x + radii[1].x), ratio(width, radii[3].x + radii[2].x), ratio(height, radii[0].y + radii[3].y), ratio(height, radii[1].y + radii[2].y));
		return paths.map((point, index) => {
			...
			const resize = (v) => new Vector(x + (v.x - corner.x) * sx, y + (v.y - corner.y) * sy);
			return new BezierCurve(resize(point.start), resize(point.startControl), resize(point.endControl), resize(point.end));
		});
	};
	//#endregion
	//#region src/render/canvas/box-shadow-transform.ts
	const shadowSpace = (matrix, width, height) => {
		...
		const corners = [
			inversePoint(0, 0),
			inversePoint(width, 0),
			inversePoint(width, height),
			inversePoint(0, height)
		];
	...
	//#endregion
	//#region src/render/canvas/box-shadow-painter.ts
	const complement = (ctx, rect, hole) => {
		ctx.beginPath();
	...
	const paintShadowSurface = async (ctx, paint, shadow, options, budget, space) => {
		const padding = paddingBox(paint.container);
		const blur = shadow.blur.number / 2;
		const margin = Math.ceil(3 * blur) + 2;
		const spread = shadow.spread.number;
		const bounds = cropSurface((shadow.inset ? padding : paint.container.bounds.add(shadow.offsetX.number - spread, shadow.offsetY.number - spread, 2 * spread, 2 * spread)).add(-margin, -margin, 2 * margin, 2 * margin), space.viewport, margin, space.scale);
		...
		try {
			...
			const path = transformPath(shadow.inset ? insetHole(paint, shadow) : spreadShadowPath(calculateBorderBoxPath(paint.curves), spread), shadow.offsetX.number, shadow.offsetY.number, 0, 0);
			...
			try {
				...
				ctx.drawImage(filtered, bounds.left, bounds.top, filtered.width / space.scale, filtered.height / space.scale);
			} finally {
				ctx.restore();
			}
			return true;
	...
	const paintBoxShadow = async (ctx, paint, shadow, options, budget) => {
		...
		const matrix = knownTransform ? ctx.getTransform() : {
			a: options.scale,
			b: 0,
			c: 0,
			d: options.scale,
		...
		if (shadow.blur.number > 0 && (shadow.inset || !space.uniform) && await paintShadowSurface(ctx, paint, shadow, options, budget, space)) return;
		const padding = paddingBox(paint.container);
		ctx.save();
		try {
			...
			const sourceBounds = shadow.inset ? padding.add(-margin, -margin, 2 * margin, 2 * margin) : paint.container.bounds.add(-spread, -spread, 2 * spread, 2 * spread);
			const { a, b, c, d, e, f } = matrix;
			const right = sourceBounds.left + sourceBounds.width;
			const bottom = sourceBounds.top + sourceBounds.height;
			const maxX = Math.max(a * sourceBounds.left + c * sourceBounds.top + e, a * right + c * sourceBounds.top + e, a * right + c * bottom + e, a * sourceBounds.left + c * bottom + e);
	...
	*/
	...
	exports.html2canvas = html2canvas;
      ...
      })(__exp, { exports: __exp }, undefined);
      return __exp.default || __exp.html2canvas;
    })();
  /* ======================================================================
   *  Modern capture layer
   *  Исправляет то, из-за чего html2canvas "криво" снимает современные сайты:
   *   1. CSS-анимации / Web Animations (framer-motion, GSAP, AOS, scroll-driven)
   *      перезапускаются в клоне с нуля -> элементы пропадают/съезжают.
   *      Решение: снимок текущих анимированных значений и фиксация их в клоне.
   *   2. @starting-style + transition, smooth-scroll -> неверные позиции.
   *   3. content-visibility:auto -> пустые блоки ниже первого экрана.
   *   4. loading="lazy" картинки вне экрана -> пустые места.
   *   5. Шрифты, подключённые через JS (FontFace API) -> не попадают в клон.
   *   6. color-mix(), light-dark(), oklch(), lab(), color(), relative colors,
   *      Tailwind v4 / shadcn палитры -> ошибка парсера или чёрные/прозрачные цвета.
  ...
  var COLOR_PROPS = [
    ...
    'border-image-source', 'outline-color', 'text-decoration-color', 'text-emphasis-color',
    'column-rule-color', 'box-shadow', 'text-shadow', 'filter',
    'fill', 'stroke', 'stop-color', 'flood-color', 'lighting-color',
    '-webkit-text-fill-color', '-webkit-text-stroke-color', 'accent-color', 'caret-color'
  ];

  // Любая "не-rgb" цветовая функция — переводим в rgba средствами самого браузера.
  var COLOR_FN_RE = /(^|[^\w-])(color-mix|light-dark|contrast-color|oklab|oklch|lab|lch|hwb|color|device-cmyk)\(/gi;
  ...
  // Обход элементов, включая содержимое shadow root.
  ...
  function queryAllDeep(root, selector) {
    var out = [];
    walk(root, function (el) { if (el.matches && el.matches(selector)) out.push(el); });
    return out;
  }
  ...
  /* ---------- 1. Снимок анимаций ---------- */
  function snapshotAnimations(doc, element) {
    var snapshots = [];
    var getAll = doc.getAnimations ? function () { return doc.getAnimations(); } : null;
    if (!getAll) return snapshots;
    var anims;
    try { anims = getAll(); } catch (e) { return snapshots; }

    var byTarget = new Map();
    anims.forEach(function (anim) {
      var effect = anim.effect;
      if (!effect || !effect.target || effect.pseudoElement) return;
      var t = effect.target;
      if (!(composedContains(element, t) || composedContains(t, element))) return;
      var props = byTarget.get(t);
      if (!props) { props = new Set(); byTarget.set(t, props); }
      try {
        effect.getKeyframes().forEach(function (kf) {
      ...
      // transition через CSSTransition тоже имеет transitionProperty
      if (anim.transitionProperty) props.add(anim.transitionProperty);
    });

    byTarget.forEach(function (props, target) {
      ...
      var uid = 'a' + (++uidCounter);
      var had = target.getAttribute(UID_ATTR);
      target.setAttribute(UID_ATTR, uid);
      snapshots.push({ uid: uid, target: target, values: values, prevAttr: had });
    });
    return snapshots;
  }

  function applyAnimationSnapshots(clonedDoc, snapshots) {
    if (!snapshots.length) return;
    var map = {};
    snapshots.forEach(function (s) { map[s.uid] = s; });
    queryAllDeep(clonedDoc.documentElement, '[' + UID_ATTR + ']').forEach(function (el) {
      var s = map[el.getAttribute(UID_ATTR)];
      if (!s) return;
      el.style.setProperty('animation', 'none', 'important');
      el.style.setProperty('transition', 'none', 'important');
      Object.keys(s.values).forEach(function (p) {
        try { el.style.setProperty(p, s.values[p], 'important'); } catch (e) {}
      });
    });
  }
  ...
  function cleanupSnapshots(snapshots) {
    snapshots.forEach(function (s) {
      if (s.prevAttr === null) s.target.removeAttribute(UID_ATTR);
      else s.target.setAttribute(UID_ATTR, s.prevAttr);
    });
  }

  /* ---------- 4. Ленивые картинки ---------- */
  function forceLazyImages(root, timeout) {
    var restored = [];
    var waits = [];
    walk(root, function (el) {
      ...
      } else if (el.decode && el.naturalWidth) {
        waits.push(el.decode().catch(function () {}));
      }
    });
    return withTimeout(Promise.all(waits), timeout).then(function () {
      return function restore() {
        restored.forEach(function (el) { el.setAttribute('loading', 'lazy'); });
      };
    });
  }
  ...
  function makeColorResolver(doc) {
    ...
    return function resolve(str) {
      ...
      if (ctx) {
        ...
        ctx.fillStyle = str;
        var b = ctx.fillStyle;
        if (!(a === '#010203' && b === '#030201')) {
          ctx.clearRect(0, 0, 1, 1);
          ctx.fillRect(0, 0, 1, 1);
          var d = ctx.getImageData(0, 0, 1, 1).data;
          var alpha = Math.round((d[3] / 255) * 1000) / 1000;
          out = alpha >= 1
            ? 'rgb(' + d[0] + ', ' + d[1] + ', ' + d[2] + ')'
  ...
  function normalizeColors(clonedDoc, clonedEl) {
    ...
    var resolve = makeColorResolver(clonedDoc);
    function fixEl(el) {
      var cs;
      try { cs = win.getComputedStyle(el); } catch (e) { return; }
      for (var k = 0; k < COLOR_PROPS.length; k++) {
        var p = COLOR_PROPS[k];
        var v = cs.getPropertyValue(p);
        if (!v || v.indexOf('(') === -1) continue;
        var fixed = replaceColorFunctions(v, resolve);
  ...
  function injectCloneStyle(clonedDoc, fix, nonce) {
    var css = 'html,body,*{scroll-behavior:auto!important}' +
      '*,*::before,*::after{transition:none!important;transition-duration:0s!important}';
    if (fix.contentVisibility) css += '*{content-visibility:visible!important}';
  ...
  function html2canvas(element, options) {
    ...
    var fix = Object.assign({}, FIX_DEFAULTS, options.fixes || {});
    delete options.fixes;

    if (!element || element.nodeType !== 1) {
      return Promise.reject(new Error('html2canvas: element must be an Element'));
    }
    ...
    var captureRoot = (element === doc.body || element === doc.documentElement) ? doc.documentElement : element;
    ...
    if (fix.lazyImages) {
      prep = prep.then(function () { return forceLazyImages(captureRoot, fix.waitTimeout); })
        .then(function (restore) { restoreLazy = restore; });
    }
    if (fix.fonts && doc.fonts && doc.fonts.ready) {
      prep = prep.then(function () { return withTimeout(doc.fonts.ready, fix.waitTimeout); });
    }
    prep = prep.then(function () {
      // Два кадра, чтобы раскладка после eager-картинок успела обновиться
      return new Promise(function (r) {
        (win.requestAnimationFrame || setTimeout)(function () { (win.requestAnimationFrame || setTimeout)(r); });
      });
    });
    if (fix.animations) {
      prep = prep.then(function () { snapshots = snapshotAnimations(doc, element); });
    }

    function cleanup() {
      cleanupSnapshots(snapshots);
      try { restoreLazy(); } catch (e) {}
    }

    return prep.then(function () { return core(element, options); })
  ...
  var TOAST_ATTR = 'data-h2c-toast';
  var MAX_SIDE = 32767, MAX_AREA = 268435456; // лимиты canvas в Chrome/Edge

  function toast(text, isError) {
    var el = document.querySelector('[' + TOAST_ATTR + ']');
    if (!el) {
      el = document.createElement('div');
      el.setAttribute(TOAST_ATTR, '');
      el.style.cssText = 'position:fixed;z-index:2147483647;right:16px;bottom:16px;padding:10px 14px;' +
        'border-radius:8px;font:14px/1.3 system-ui,sans-serif;color:#fff;box-shadow:0 4px 16px rgba(0,0,0,.3);' +
        'pointer-events:none;max-width:360px';
      document.documentElement.appendChild(el);
    }
    el.style.background = isError ? '#c62828' : '#222';
    el.textContent = text;
    return el;
  }
  ...
  function fileName() {
    ...
    var host = (location.hostname || 'page').replace(/[^\w.-]+/g, '_');
    return host + '_' + d.getFullYear() + '-' + p(d.getMonth() + 1) + '-' + p(d.getDate()) +
  ...
  function canvasToBlob(canvas) {
    return new Promise(function (resolve, reject) {
      try {
        canvas.toBlob(function (b) { b ? resolve(b) : reject(new Error('toBlob вернул пустой результат')); }, 'image/png');
      } catch (e) { reject(e); }
    });
  }
  ...
  var opts = {
    scale: scale,
    ignoreElements: function (el) { return el.hasAttribute && el.hasAttribute(TOAST_ATTR); }
  };
  ...
  toast('📸 Снимаю ' + (CONFIG.mode === 'viewport' ? 'видимую область' : 'страницу') + '…');

  return html2canvas(root, opts)
    .then(canvasToBlob)
    .then(function (blob) {
      var name = fileName();
      download(blob, name);
      var el = toast('✅ Сохранено: ' + name + ' (' + Math.round(blob.size / 1024) + ' КБ, ' +
        ((performance.now() - t0) / 1000).toFixed(1) + ' с)');
      setTimeout(function () { el.remove(); }, 4000);
      console.log('[screenshot] сохранено', name, (w * scale | 0) + '×' + (h * scale | 0));
    })
    .catch(function (err) {
      console.error('[screenshot] ошибка:', err);
      var el = toast('❌ Не удалось сделать скриншот: ' + (err && err.message || err), true);
